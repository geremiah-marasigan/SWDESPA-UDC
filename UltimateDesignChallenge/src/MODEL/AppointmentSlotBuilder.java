...
public class AppointmentSlotBuilder extends TimeslotBuilder {
    ...
    public boolean isAvailable(Appointment app) {
        // DO CHECKING HERE
        if (app.getEndTime() <= app.getStartTime()) {
            return false;
        }

        List<Appointment> apps = ((DoctorController) controller).getAllSlots();
        for (Appointment a : apps) {
            if (checkDate(a.getStartDay(), a.getEndDay(), app.getStartDay(), app.getEndDay()) && overlap(app, a)) {
                System.out.println("TAKENNNN");
                return false;
            }
        }
    ...
    public boolean isSlot(Appointment app, List<String>doctors) {
        ...
        //System.out.println("NOT SLOT, BOOKED!");
        return false;
    } 
    public String getTarget(int time, List<Appointment>taken) {
        for (Appointment a:taken) {
            if (time >= a.getStartTime() && time <= a.getEndTime()) {
                return a.getName();
            }
        }
        return null;
