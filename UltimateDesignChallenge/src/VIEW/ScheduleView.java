...
public class ScheduleView extends JPanel {
    ...
    // DOCTOR
    public void setItems(List<Appointment> apps, String curDate, User user) {
        ...
        List<String> doctors = ((DoctorController) controller).getDoctors();
        Director director = new Director();
        director.setTimeslotBuilder(new AppointmentSlotBuilder(), controller);
        director.addSchedulePanels(items, apps, user, curDate, delapps, taken, doctors);
            ...
            for (int min = 0; min < 60; min += 30) {
                ...
                for (int x = 0; x < apps.size(); x++) {
                    if (doctor == null) {
                        ...
                        if (users != null) {
                            for (User user : users) {
                                if (apps.get(x).getName().equals(name) && user.getType().equals("DOCTOR")) {
                                    ...
                                    doctor = apps.get(x);
                                    type = "doctor";
                                } else if (apps.get(x).getName().equals(user.getLastname()) && user.getType().equals("CLIENT")) {
                                    if (doctor != null) {
                        ...
                        if (x + 1 < apps.size()) {
                            if (apps.get(x + 1).getStartTime() == apps.get(x).getEndTime()) {
                                for (User user : users) {
                                    if (apps.get(x + 1).getName().equals(name) && user.getType().equals("DOCTOR")) {
                                        temp = apps.get(x + 1);
                                        doctor = apps.get(x + 1);
                                        type = "doctor";
                                    } else if (apps.get(x + 1).getName().equals(user.getLastname()) && user.getType().equals("CLIENT")) {
                                        if (doctor != null) {
